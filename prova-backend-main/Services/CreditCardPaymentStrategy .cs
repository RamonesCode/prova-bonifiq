...
namespace ProvaPub.Services
{
    public class CreditCardPaymentStrategy : IPaymentStrategy
    {
        public async Task Pay(decimal paymentValue, int customerId)
        {
            ...
            // Realizar o pagamento via cartão de crédito
            bool pagamentoAprovado = ProcessarPagamento(paymentValue);

            // Verificar se o pagamento foi aprovado
            if (!pagamentoAprovado)
            {
                throw new Exception("O pagamento foi recusado. Por favor, verifique as informações do seu cartão de crédito e tente novamente.");
            }
